...
def quicksort(arr, key_func):
    ...
    pivot = key_func(arr[len(arr) // 2])
    left = [x for x in arr if key_func(x) < pivot]
...
if __name__ == "__main__":
    ...
    output_file = os.path.join(output_dir, "ordenado_quicksort.bib")
    ...
    print("[INFO] Ordenando con QuickSort por (año, título)...")
    start_time = time.time()  # <-- inicio tiempo
    sorted_entries = quicksort(entries, sort_key)
